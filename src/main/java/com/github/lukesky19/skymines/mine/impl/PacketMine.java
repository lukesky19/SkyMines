...
import com.github.lukesky19.skymines.mine.data.PacketBlock;
...
public class PacketMine implements Mine {
     ...
     * You can use {@link #isBlockMineable(Player, Location, BlockType)} for checking child regions.
    ...
    public boolean isBlockMineable(@NonNull Player player, @NonNull Location location, @NonNull BlockType blockType) {
        ...
        if(!mineTimeManager.hasMineTime(player.getUniqueId(), mineId)) return false;
    ...
    public void handleBlockBreak(@NonNull BlockBreakEvent blockBreakEvent) {
        ...
        if(isLocationOnCooldown(uuid, location)) {
            ...
            if(!isBlockMineable(player, location, blockType)) {
                blockBreakEvent.setCancelled(true);
